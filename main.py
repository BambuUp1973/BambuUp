...
- File grafici: quello che CHIEDIAMO è il vettoriale (.AI, .EPS, .PDF, .SVG). Se il cliente non ce l'ha, il raster (JPEG, PNG) si accetta lo stesso e la conversione la facciamo noi con un costo di 25 euro A IMMAGINE, cioè a logo: tre loghi raster sono tre conversioni, 75 euro. Non rifiutare MAI un ordine perché il cliente ha solo un JPEG: proponi la conversione. Quando le immagini sono più di una si può valutare un forfait caso per caso in base al progetto, ma la decisione sullo sconto la prende chi gestisce l'ordine, non tu: al cliente (anche business) non promettere mai il forfait come se fosse un prezzo, digli che "di volta in volta valutiamo se c'è la possibilità di un forfait in base al tipo di progetto"
